...
| `create` | `object` | Generates `Create`. Supports `key` and `batch` sub-fields. |
| `create.key` | `string` | Key provider: `server` (default, server-side generation) or `client` (client-specified). |
| `create.batch` | `bool` | Set to `true` to additionally generate a `BatchCreate<Entity>s` RPC (batch create). Default `false`. |
...
| `list` | `object` | Generates an entity-level `List`. Use `resource` to specify the target resource; `list_config` configures the filter type. |
...
When `create: { batch: true }`, an additional `BatchCreate<Entity>s` RPC is generated:

| Method | Request | Response | HTTP Path |
| --- | --- | --- | --- |
...
- `requests` elements reuse the single `Create<Entity>Request` shape (with respective resources, supporting partial create)
- `batch: true` can be combined with `key: server` / `key: client`
- HTTP path does not carry key (collection-level operation, keys are inside the body's requests)
- Do not remove `batch` after release (removal causes the RPC to disappear, which is a breaking change)
...
#### Entity-Level List (`list`)

`list` is an entity-level capability that targets a set of resources via `resource` (a list). It does not require a specific key, and its HTTP path is `/{prefix}/{Service}/{collection}/list` (collection-level operation, no resource or key segment).

| Field | Type | Purpose |
| --- | --- | --- |
| `list.resource` | `[]string` | Resources covered by the list query; each must be a resource declared on the entity, non-empty and non-duplicated. |
| `list.list_config.filter_type` | `string` | Optional type reference for the `filter` field. |

Generated RPC `List<Entity>s` (e.g. `ListBooks`), request `List<Entity>sRequest`: `limit = 1`, `offset = 2`, `filter = 3` (when `list_config.filter_type` is set), `order_by = 4`; response `List<Entity>sResponse`: `repeated <Entity>Item items = 1`, `total_size = 2` (required). An `<Entity>Item` aggregate type is auto-generated (e.g. `BookItem`) whose fields are each declared resource in order (`meta=1, content=2`). Pagination uses offset style rather than cursor (page_token).
...
| `services[].entities[].list` | `bool` | Optional. Set to `false` to narrow out the entity's `List`; when omitted or `true`, inherits the entity `list` config. |
| `services[].entities[].resources` | `[]object` | Optional narrowing rules; currently uses resource `name`, `reader.batch`, and `writer.update`. Omitting inherits the entity's full capabilities. |
...
services:
  ...
  - name: AdminService
    entities:
      - name: book
        list: true               # Expose entity-level ListBooks
    custom_methods:
      - name: ArchiveBook
